...
def draw_boxes(image_raw, result_boxes, result_scores, result_classid):
    ...
    for i in range(len(result_boxes)):
        ...
        plot_one_box(
            ...
            image_raw,
            label="{}:{:.2f},fps:{}".format(
                categories[int(result_classid[i])], result_scores[i],1.0/(t2-t1)
            ),
        )
    return image_raw
...
def click(event,x,y,flag,params):
    global click_x1,click_x2,click_y1,click_y2,goflag
...
def detect_camera(camera, yolov5_wrapper):
    ...
    cv2.setMouseCallback("Origin", click)
    ...
    while True:
        ...
        #ROI
        if goflag == 1:
            img_box = cv2.rectangle(img, (click_x1,click_y1), (click_x2,click_y2), (0,0,255),thickness)
            cv2.namedWindow("Origin with Box")
            cv2.imshow("Origin with Box",img_box)
            cv2.moveWindow("Origin with Box",640,0)
            roi = img[click_y1+thickness:click_y2-thickness,click_x1+thickness:click_x2-thickness]
            roi_x, roi_y = roi.shape[0:2]
            roi = cv2.resize(roi,(int(roi_y*2),int(roi_x*2)))
            roi, result_boxes, result_scores, result_classid = yolov5_wrapper.infer(roi)

            
            t2 = time.time()
            GPIO.output(21,False)

            roi = draw_boxes(roi, result_boxes, result_scores, result_classid)

            cv2.namedWindow("roi", 0)
            cv2.resizeWindow("roi", roi_y*2, roi_x*2)
            cv2.imshow("roi", roi)
            cv2.moveWindow("roi",1280,0)
